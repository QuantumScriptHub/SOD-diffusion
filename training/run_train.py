    ...
    with torch.no_grad():
        for img_name in os.listdir(args.val_img):
            ...
            pipe_out = pipeline(input_image_pil,
                                ...
                                swinb_path=os.path.join(args.output_dir, f"swin_ft_{epoch}.pth"),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to pretrained swinb model.",
    )
    parser.add_argument(
        "--train_img_list",
        type=str,
...
def main():
    ...
    sod_backbone = SalientBranch_SwinB(64, False, [384, 384])
    sod_backbone.load_state_dict(torch.load(args.swinb_model_path), strict=True)
    ...
    # optimizer
    downsample = lambda x, scale_factor: F.interpolate(x, scale_factor=scale_factor, mode='bilinear', align_corners=False)
    swin_feat_conv = torch.nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1)
    torch.nn.init.kaiming_normal_(swin_feat_conv.weight)
    optimizer_cls = torch.optim.AdamW
    optimizer = optimizer_cls(
        list(unet.parameters()) + list(sod_backbone.parameters()) + list(swin_feat_conv.parameters()),
    ...
    # Move vae and sod_backbone to gpu and cast to weight_dtype
    vae.to(accelerator.device, dtype=weight_dtype)
    sod_backbone.to(accelerator.device, dtype=weight_dtype)
    swin_feat_conv.to(accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with (accelerator.accumulate(unet)):
                ...
                # encode swinb to cross_kv
                swin_feat = sod_backbone.forward_encode(salient_image.to(weight_dtype))
                swin_feat_out = swin_feat_conv(swin_feat)
                ...
                noise_pred = unet(unet_input, timesteps, encoder_hidden_states=swin_feat).sample
                loss = F.mse_loss(noise_pred.float(), target.float()) + F.mse_loss(
                    swin_feat_out.float(), downsample(salient_gt, 0.125))
        ...
        if accelerator.is_main_process:
            torch.save(sod_backbone.state_dict(), os.path.join(args.output_dir, f"swin_ft_{epoch}.pth"))
            log_validation(
                vae=vae,
                unet=unet,
                args=args,
